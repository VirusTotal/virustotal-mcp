...
OpenAI documents OAuth 2.1 for authenticated hosted MCP. VTAI provides OAuth separately from its static token headers and supports DCR and CIMD. On 2026-09-24, ChatGPT Work completed personal OAuth consent, returned an IP report through the production MCP server, then automatically renewed its token and returned a subsequent report after the previous token had expired. Follow the [ChatGPT setup guide](hosted-clients.md#connect-chatgpt-work): creating the application and connecting your personal account are separate steps. Leave optional OAuth client credentials blank; a VTAI token is not an OAuth client secret. Revocation from the hosted application, incremental authorization and write workflows remain unverified. See [OAuth compatibility](hosted-clients.md#oauth-compatibility) and [OpenAI authentication requirements](https://developers.openai.com/plugins/build/auth).
...
The following rows and session descriptions retain their recorded scope and versions. Pending checks describe those observations; current coverage for the three native clients is listed above. The ChatGPT Work row records its separate 2026-09-24 hosted validation.
...
| ChatGPT Work (hosted) | Web, 2026-09-24 | Production HTTP / OAuth CIMD + PKCE | Personal browser consent completed | Initial IP report and subsequent report after token expiry | Initial read and automatic renewal verified; hosted revocation, incremental authorization and writes unverified |
...
These historical CLI and IDE sessions do not establish hosted connections or external catalog acceptance. ChatGPT's later OAuth connection, report reads and automatic renewal are recorded in the [hosted validation scope](hosted-clients.md#validation-scope); Claude hosted acceptance remains unverified. Gemini HTTP calls and model workflows remain pending. Claude Code's later HTTP workflow is covered in the latest native-client record above.
