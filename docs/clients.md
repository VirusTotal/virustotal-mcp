...
OpenAI documents OAuth 2.1 for authenticated hosted MCP. VTAI provides OAuth separately from its static token headers and supports DCR and CIMD. On 2026-09-24, ChatGPT Work completed personal OAuth consent and returned an IP report through the production MCP server. Follow the [ChatGPT setup guide](hosted-clients.md#connect-chatgpt-work): creating the application and connecting your personal account are separate steps. Leave optional OAuth client credentials blank; a VTAI token is not an OAuth client secret. Hosted refresh, revocation, incremental authorization and write workflows remain unverified. See [OAuth compatibility](hosted-clients.md#oauth-compatibility) and [OpenAI authentication requirements](https://developers.openai.com/plugins/build/auth).
...
| ChatGPT Work (hosted) | Web, 2026-09-24 | Production HTTP / OAuth CIMD + PKCE | Personal browser consent completed | One IP report returned through MCP | Initial hosted read verified; refresh, revocation, incremental authorization and writes unverified |
