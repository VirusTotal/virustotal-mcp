...
SHA = "a" * 40
PACKAGE_SOURCE_SHA = "03c452bb80dbcbb7ae6b30d151ca4f118f929cce"
...
def manifest(repository, version=None):
    ...
    if version in {"0.8.5", "0.8.6"}:
        result["packages"][0]["version"] = "0.8.4"
    if version == "0.8.7":
        result["packages"][0]["version"] = "0.8.5"
    if version == "0.9.0":
        result["packages"][0]["version"] = "0.9.1"
...
def harness(tmp_path, monkeypatch):
    ...
    state = SimpleNamespace(
        ...
        previous={
            ...
            for version in ("0.8.2", "0.8.3", "0.8.4", "0.8.5", "0.8.6", "0.8.7", "0.9.0")
        },
        pypi_reads=[],
        gh_override={},
        claims={},
    ...
    state.checksums = (
        f"{'1' * 64}  vt_mcp-0.9.2-py3-none-any.whl\n{'2' * 64}  vt_mcp-0.9.2.tar.gz\n"
    ).encode()
    ...
    state.pypi = {
        ...
        "urls": [
            {
                "filename": "vt_mcp-0.9.2-py3-none-any.whl",
                "packagetype": "bdist_wheel",
                "digests": {"sha256": "1" * 64},
                "yanked": False,
            },
            {
                "filename": "vt_mcp-0.9.2.tar.gz",
                "packagetype": "sdist",
                "digests": {"sha256": "2" * 64},
                "yanked": False,
            },
    ...
    state.release_responses = {
        ...
        f"repos/{CORPORATE}/releases/tags/v0.9.2": {
            ...
            "assets": [
                ...
                {
                    "id": 2,
                    "name": "vt_mcp-0.9.2-py3-none-any.whl",
                    "state": "uploaded",
                    "digest": f"sha256:{'1' * 64}",
                },
                {
                    "id": 3,
                    "name": "vt_mcp-0.9.2.tar.gz",
                    "state": "uploaded",
...
def test_verify_identity_authenticates_but_never_changes_entries(harness, capsys):
    ...
    assert result["before"] == result["after"] and len(harness.reads) == 22
...
def test_publish_keeps_previous_manifests_and_uses_existing_package(harness):
    ...
    for version, manifest_sha in (
        ...
        ("0.8.6", "6a5600b9d522ea989228caae71ad3f3250431f76516297dbd41b7cc2f462782e"),
        ("0.8.7", "c85f2391f0306c21bb5e55f11c78c4aa33b599f0278ac430af42c184d1821b99"),
    ...
    assert result["after"][CORPORATE]["version"] == "0.9.1"
    assert result["pypi"]["version"] == "0.9.2"
    assert result["pypi"]["source_sha"] == PACKAGE_SOURCE_SHA != result["sha"]
    assert result["pypi"]["files"] == {
        "vt_mcp-0.9.2-py3-none-any.whl": "1" * 64,
        "vt_mcp-0.9.2.tar.gz": "2" * 64,
    }
    assert harness.pypi_reads == ["0.9.2"]
...
@pytest.mark.parametrize("version", ["0.8.2", "0.8.3", "0.8.4", "0.8.5", "0.8.6", "0.8.7", "0.9.0"])
...
@pytest.mark.parametrize("version", ["0.8.1", "0.8.8"])
...
def test_release_provenance_must_match_public_package(harness, problem):
    ...
    release = harness.release_responses[f"{prefix}/releases/tags/v0.9.2"]
    if problem == "lightweight_tag":
        harness.release_responses[f"{prefix}/git/ref/tags/v0.9.2"]["object"]["type"] = "commit"
    elif problem == "moved_tag":
        harness.release_responses[f"{prefix}/git/ref/tags/v0.9.2"]["object"]["sha"] = "c" * 40
...
def test_manifest_uses_only_a_path_for_stdio_credential():
    ...
    assert package["version"] == "0.9.2" and value["version"] == "0.9.1"
